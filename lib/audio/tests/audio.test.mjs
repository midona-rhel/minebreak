  ...
  for (const instrument of [
    ...
    const event = {
      ...
      midi:
        instrument === 'sub'
          ? 28
          : instrument === 'growl' || instrument === 'bass'
            ? 40
            : 74,
      duration: 0.9,
      velocity: 0.7,
...
test('the production render has headroom, dynamics, stereo space and a continuous seam', () => {
  ...
  assert.ok(
    sections[2].rms > sections[0].rms * 1.8 &&
      sections[5].rms > sections[3].rms * 1.8,
    'both bass drops must clearly lift above the intro and breakdown',
  );
  ...
  if (process.env.MINEBREAK_RENDER_MUSIC === '1') {
    ...
    writeFileSync('work/music/lanterns-after-dark.wav', wav);
    writeFileSync(
