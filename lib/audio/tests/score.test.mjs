...
test('the lead has breathing space, a clear reprise, and a quieter middle', () => {
  ...
  const opening = inBars(0, 2),
    reprise = inBars(64, 66);
  assert.deepEqual(
    opening.map((n) => n.midi),
    reprise.map((n) => n.midi),
  );
  assert.ok(
    inBars(32, 40).length < inBars(0, 8).length,
    'breakdown leaves more space',
  );
  assert.ok(
  ...
  assert.equal(
    score.filter(
      (n) =>
        n.instrument === 'kick' && n.start >= 32 * BAR && n.start < 40 * BAR,
    ).length,
    0,
  );
});

test('both drops have a clean sub, authored wobble phrases and a half-time backbeat', () => {
  const score = createScore();
  for (const first of [16, 48]) {
    const section = score.filter(
      (n) => n.start >= first * BAR && n.start < (first + 16) * BAR,
    );
    assert.ok(section.filter((n) => n.instrument === 'sub').length > 40);
    assert.ok(
      section
        .filter((n) => n.instrument === 'growl')
        .every((n) => n.wobble >= 2.4 && n.wobble <= 14.4),
    );
    for (let bar = first; bar < first + 16; bar++)
      assert.ok(
        section.some(
          (n) =>
            n.instrument === 'snare' &&
            Math.abs(n.start - (bar * BAR + 4 * EIGHTH)) < 1e-8,
        ),
      );
    assert.equal(
      section.filter((n) => n.instrument === 'lute').length,
      0,
      'the drop makes room for the bass',
    );
  }
});
