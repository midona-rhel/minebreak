...
Open /wackdonalds for an immediate practice shift. After each completed shift, a receipt shows food score, meals completed, salad incidents (plant strikes), time worked, health lost and XP earned. Another Shift clears the receipt, restores 5 health and selects a fresh seed. The replay button receives focus when the encounter closes. Cancelling returns to the free-play screen without a receipt.

The receipt is free-play-only. Main-run encounters keep their existing immediate return to the board. Practice uses disposable stats, does not write board progress, and shows actual earned XP rather than treating the food score as currency. The growth increase also applies to main-run Wackdonalds, but food points, the 48-point target, and the XP formula are unchanged.

Receipt regression tests cover successful shifts, bombs, timeouts, capped XP, snapshot immutability and unfinished/fresh shifts. Free-play browser interaction, receipt appearance and replay focus still need manual confirmation; unit tests and compilation do not establish those results.
## Art provenance
