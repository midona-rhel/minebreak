...
export default function MinebreakGame() {
  ...
  const finish = useCallback((result: EncounterResult) => { if (!encounter) return; setCells((all) => all.map((c) => c.id === encounter.id ? { ...c, open: true, disarmed: true } : c)); const reportedDamage = Math.max(0, Math.min(hp, Math.ceil(result.damageTaken ?? (result.outcome === 'failure' ? hp : 0)))); const damage = Math.max(0, reportedDamage - perks.armor); const health = hp - damage; setHp(health); if (result.outcome === 'success') { setXp((v) => v + 35 + floor * 5); save({ ...profile, disarmed: profile.disarmed + 1 }); setMessage(damage ? `Mine disabled. Lost ${damage} integrity in the duel.` : 'Mine disabled without damage.'); } else { setMessage(`Duel lost. Lost ${damage} integrity.`); if (health <= 0) setPhase('dead'); } setEncounter(null); }, [encounter, floor, hp, perks.armor, profile, save]);
  ...
  return <main className="mb-shell"><ThreeScene danger={Boolean(encounter)} /><header className="mb-top"><div className="mb-brand"><span><Pickaxe /></span><b>MINE<i>BREAK</i></b><small>ROGUELIKE PROTOCOL</small></div><div className="sector"><i /> SECTOR {String(floor).padStart(2, '0')} <span>DEPTH {floor * 120}M</span></div><button onClick={newRun}><RotateCcw /> NEW RUN</button></header>
    ...
    {encounter && <EncounterHost key={`${seed}:${floor}:${encounter.id}`} definition={selectMinigame(seed + floor * 101 + encounter.id)} context={{ seed: seed + floor * 101 + encounter.id, floor, cellId: encounter.id }} player={{ currentHp: hp, maxHp }} complete={finish} cancel={() => { setCells(all => all.map(c => c.id === encounter.id ? { ...c, open: false } : c)); setEncounter(null); setMessage('Returned to board.'); }} />}
